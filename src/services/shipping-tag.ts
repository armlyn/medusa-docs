...
export default class ShippingTagService extends TransactionBaseService {
  ...
  async getTestDisplayNumber(
    ...
    const nextNumber: string | undefined =
      forcedNumber !== undefined
        ? forcedNumber
        : await this.getNextShippingTagNumber();
    if (nextNumber) {
      return formatNumber
        ? formatNumber.replace(SHIPPING_TAG_NUMBER_PLACEHOLDER, nextNumber)
        : nextNumber;
    }
    throw new MedusaError(
      MedusaError.Types.INVALID_DATA,
  ...
  async generateTestShippingTag(
    templateKind: ShippingTagTemplateKind
  ): Promise<ShippingTagResult> {
    ...
    if (lastOrder && lastOrder.length > 0) {
      ...
      if (settings) {
        const shippingTagSettings: DocumentShippingTagSettings | undefined =
          await this.documentShippingTagSettingsService.getDocumentShippingTagSettings();
        if (shippingTagSettings) {
          const testShippingTag = this.activeManager_.create(ShippingTag);
          const nextNumber: string = await this.getNextShippingTagNumber();
          testShippingTag.number = nextNumber;
          testShippingTag.display_number =
            shippingTagSettings.shipping_tag_number_format
              ? shippingTagSettings.shipping_tag_number_format.replace(
                  SHIPPING_TAG_NUMBER_PLACEHOLDER,
                  testShippingTag.number
                )
              : testShippingTag.number;
          testShippingTag.created_at = new Date(Date.now());
          const [validationPassed, info] = validateInputForProvidedKind(
            templateKind,
            settings
          );
          ...
          throw new MedusaError(
            MedusaError.Types.INVALID_DATA,
            "Shipping tag settings are not defined"
          );
        }
      } else {
        throw new MedusaError(
